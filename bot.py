...
def farsi_title_and_summary(cluster: StoryCluster) -> tuple[str, str, str]:
    ...
    if re.search(r"organ donation|donat(?:e|ion).*organ|saves two lives|پیوند عضو|اهدای عضو", text, re.I):
        title = "اهدای عضو زن هندی در دبی جان دو نفر را نجات داد"
        summary = "یک مادربزرگ هندی در دبی پس از درگذشت، با اهدای اعضای بدن خود جان دو نفر را نجات داد. خانواده او با اهدای عضو موافقت کردند و این اقدام باعث شد بیماران نیازمند فرصت تازه ای برای ادامه زندگی پیدا کنند."
        caption = "اهدای عضو یک زن هندی در دبی جان دو نفر را نجات داد."
    elif re.search(r"textile sector|textiles|قطاع النسيج|نساجی|منسوجات", text, re.I):
        title = "ابتکار تازه امارات برای تحول صنعت نساجی"
        summary = "رئیس امارات ابتکاری تازه برای تحول بخش نساجی کشور اعلام کرده است. این برنامه با هدف توسعه صنعت منسوجات، تقویت تولید و ایجاد فرصت های تازه در حوزه پوشاک و پارچه معرفی شده است."
        caption = "امارات برنامه تازه ای برای تحول صنعت نساجی اعلام کرد."
    elif re.search(r"al-aqsa|aqsa|settlers|israeli settlers|الأقصى|الاقصى|شهرک نشین", text, re.I):
        title = "محکومیت ورود شهرک نشینان به مسجدالاقصی از سوی امارات"
        summary = "امارات ورود شهرک نشینان اسرائیلی به محوطه مسجدالاقصی را به شدت محکوم کرده است. در موضع رسمی منتشرشده، امارات بر ضرورت احترام به اماکن مقدس و جلوگیری از تشدید تنش ها تاکید کرده است."
        caption = "امارات ورود شهرک نشینان به مسجدالاقصی را محکوم کرد."
    elif re.search(r"police station.*safety|safety rate|response|نسبة الأمان|مركز شرطة", text, re.I):
        title = "ثبت شاخص ایمنی بالا در یکی از مراکز پلیس دبی"
        summary = "یکی از مراکز پلیس دبی شاخص ایمنی ۹۸.۹ درصدی و زمان پاسخگویی ۱.۳۸ دقیقه ای ثبت کرده است. این آمار در گزارش عملکرد پلیس دبی آمده و به خدمات امنیتی و پاسخگویی سریع در محدوده آن مرکز مربوط می شود."
        caption = "یکی از مراکز پلیس دبی شاخص ایمنی بسیار بالا ثبت کرد."
    elif re.search(r"chicken imports|unpaid chicken|dh14\.?3|14\.?3 million|imports", text, re.I):
        title = "حکم دادگاه دبی درباره پرداخت بدهی واردات مرغ"
        summary = "دادگاه دبی در یک پرونده تجاری مربوط به واردات مرغ، طرف بدهکار را به پرداخت ۱۴.۳ میلیون درهم محکوم کرده است. پرونده به اختلاف بر سر مبالغ پرداخت نشده واردات مربوط بود و دادگاه حکم به پرداخت مبلغ مطالبه شده داد."
        caption = "دادگاه دبی در پرونده واردات مرغ حکم پرداخت ۱۴.۳ میلیون درهم صادر کرد."
        ...
        if re.search(r"oud|عود", text, re.I) and re.search(r"theft|stole|steal|stealing|سرق|سرقت", text, re.I):
            ...
            summary = "یک خبر تازه پلیسی یا امنیتی در امارات منتشر شده است. مقام های رسمی درباره یک پرونده، آمار امنیتی، حادثه یا اقدام پلیسی اطلاع رسانی کرده اند."
        ...
        if re.search(r"eswatini|إسواتيني|اسواتيني|visa waiver|mutual visa|الإعفاء المتبادل|تأشيرة الدخول", text, re.I):
            ...
            summary = "یک تصمیم یا اطلاعیه رسمی تازه درباره قوانین، ویزا، جریمه ها، مجوزها، پارکینگ، سالک یا خدمات شهری امارات منتشر شده است. جزئیات خبر به تغییر یا یادآوری یک خدمت یا قانون رسمی مربوط می شود."
        ...
        if re.search(r"sunset|romantic|restaurants.*sunset|رمانتیک|غروب", text, re.I):
            title = "رستوران های دبی برای تماشای غروب"
            summary = "این خبر چند رستوران در دبی را معرفی می کند که برای تماشای غروب و قرارهای رمانتیک انتخاب شده اند. تمرکز خبر روی رستوران هایی است که منظره، فضای آرام و تجربه شام در زمان غروب را ارائه می دهند."
            caption = "چند رستوران دبی برای شام و تماشای غروب معرفی شدند."
        elif re.search(r"cafe|caf[eé]|alserkal", text, re.I):
            title = "کافه های دیدنی دبی برای لیست آخر هفته"
            summary = "این خبر چند کافه یا تجربه غذایی در دبی را معرفی می کند. تمرکز خبر روی مکان هایی است که برای قهوه، قرار، فضای شهری و تجربه تازه در دبی قابل انتخاب هستند."
            ...
            summary = "این خبر چند رستوران و استیک هاوس در دبی را معرفی می کند؛ از گزینه های لوکس و واگیو گرفته تا تجربه های مناسب شام و مناسبت های خاص."
            ...
            summary = "این خبر چند برانچ و تجربه غذایی آخر هفته در دبی را معرفی می کند. موضوع خبر به انتخاب رستوران، فضای غذاخوری و برنامه های آخر هفته مربوط است."
            ...
            summary = "این خبر یک بار، رستوران یا فضای شبانه در دبی را معرفی می کند. جزئیات خبر به فضای مکان، تجربه غذا و نوشیدنی و مناسب بودن آن برای قرار یا دورهمی مربوط است."
            ...
            summary = "این خبر چند استخر، ریزورت یا تجربه فضای باز در دبی را معرفی می کند. موضوع خبر به برنامه آخر هفته، فضای آفتابی و لوکیشن های تفریحی شهر مربوط است."
            caption = "یک ایده تصویری و جذاب برای آخر هفته در دبی."
        else:
            title = "پیشنهاد تازه برای سبک زندگی در دبی"
            summary = "این خبر یک پیشنهاد مشخص از سبک زندگی دبی را معرفی می کند؛ مانند رستوران، کافه، رویداد، مرکز خرید، پاپ آپ یا برنامه آخر هفته."
        ...
        if re.search(r"paragliding|ice-cream|ice cream|landlord", text, re.I):
            ...
            summary = "این خبر درباره یک سوژه اجتماعی، تصویری یا پربحث در دبی است. موضوع خبر به اتفاقی مربوط است که در فضای عمومی یا آنلاین توجه گرفته است."
        ...
        if re.search(r"gdp|economy|اقتصاد|الناتج المحلي|6\\.2|6,2|1\\.9|1,9|trillion|تريليون", text, re.I):
            ...
            summary = "این خبر به اقتصاد دبی، بازار ملک، سرمایه گذاری، استارتاپ ها، فرصت های شغلی یا فضای کسب وکار مربوط است. جزئیات خبر به یک عدد، تصمیم، شرکت، پروژه یا بخش اقتصادی تازه اشاره دارد."
            caption = "یک خبر تازه از اقتصاد و کسب وکار دبی."
    else:
        title = "به روزرسانی تازه از دبی و امارات"
        summary = "یک خبر تازه درباره دبی یا امارات منتشر شده است. جزئیات این خبر به یک اعلامیه، رویداد، تصمیم یا اتفاق جدید در امارات مربوط می شود."
