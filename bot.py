...
def farsi_brief(cluster: StoryCluster) -> str:
    ...
    coverage = "این موضوع توسط چند منبع معتبر پوشش داده شده و ارزش توجه بیشتری دارد. " if len(cluster.sources) > 1 else ""
    if re.search(r"fake|fraud|scam|warn|booking|کلاهبرداری|احتيال|مزيف", text, re.I):
        context = "این خبر درباره هشدار پلیس یا مقام های رسمی به ساکنان دبی درباره کلاهبرداری، پیشنهادهای جعلی یا ریسک های امنیتی است."
        angle = "زاویه پیشنهادی برای کپشن: با یک هشدار کوتاه شروع کنید، سپس بگویید مردم باید قبل از پرداخت یا رزرو چه نشانه هایی را بررسی کنند."
        ...
        context = "این یک خبر مثبت محلی از دبی است؛ داستانی درباره امانت داری، رفتار انسانی و بازگرداندن پول یا اموال به صاحبش."
        angle = "زاویه پیشنهادی برای کپشن: حس خوب شهر، اعتماد اجتماعی و واکنش پلیس یا مردم را برجسته کنید."
    elif re.search(r"solidarity|condolence|condemns|foreign|minister|تعزي|تتضامن|يدين", text, re.I):
        context = "امارات در واکنش به یک اتفاق مهم بین المللی پیام همبستگی، تسلیت یا موضع رسمی منتشر کرده است."
        angle = "زاویه پیشنهادی برای کپشن: روی جنبه انسانی خبر و نقش دیپلماسی امارات به زبان ساده تمرکز کنید."
    elif "crime" in tags:
        context = "این خبر به پلیس، دادگاه، امنیت عمومی، پرونده های قضایی یا هشدارهایی مربوط است که می تواند برای ساکنان دبی مهم باشد."
        angle = "زاویه پیشنهادی برای کپشن: ابتدا اثر مستقیم خبر روی مردم را بگویید و بعد نکته احتیاطی یا قانونی را خیلی واضح اضافه کنید."
    elif "rules" in tags:
        context = "این خبر درباره یک تغییر یا یادآوری کاربردی در قوانین، ویزا، جریمه ها، مجوزها، پارکینگ، سالک یا خدمات شهری امارات است."
        angle = "زاویه پیشنهادی برای کپشن: خیلی شفاف توضیح دهید چه چیزی تغییر کرده، از چه زمانی مهم است و چه کسانی باید حواسشان باشد."
    elif "weather/traffic" in tags:
        context = "این خبر یک اطلاع رسانی روزمره و کاربردی درباره آب وهوا، جاده ها، ترافیک، پروازها یا رفت وآمد در دبی و امارات است."
        angle = "زاویه پیشنهادی برای کپشن: زمان، مکان و کاری را که مخاطب باید انجام دهد در همان خط اول بیاورید."
    elif "lifestyle" in tags:
        context = "این خبر برای محتوای سبک زندگی دبی مناسب است؛ از رویداد و رستوران گرفته تا مراکز خرید، پاپ آپ ها و برنامه های آخر هفته."
        angle = "زاویه پیشنهادی برای کپشن: آن را مثل یک پیشنهاد کوتاه و جذاب برای تجربه کردن در دبی معرفی کنید."
    elif "viral" in tags:
        context = "این موضوع ظرفیت وایرال شدن دارد، چون می تواند بحث راه بیندازد، تصویر خوبی بسازد یا برای مخاطب قابل اشتراک گذاری باشد."
        angle = "زاویه پیشنهادی برای کپشن: با یک سوال کوتاه، جمله کنجکاوکننده یا زاویه بحث برانگیز شروع کنید تا کامنت بگیرد."
    elif "business" in tags:
        context = "این خبر به اقتصاد دبی، بازار ملک، سرمایه گذاری، استارتاپ ها، فرصت های شغلی یا فضای کسب وکار مربوط است."
        angle = "زاویه پیشنهادی برای کپشن: اثر خبر روی زندگی، پول، ملک یا تصمیم های روزمره مردم را ساده و قابل لمس توضیح دهید."
    else:
        context = "این یک به روزرسانی تازه درباره دبی یا امارات است که می تواند برای مخاطب محلی ارزش خبری یا کاربردی داشته باشد."
        angle = "زاویه پیشنهادی برای کپشن: یک برداشت کوتاه و روشن از اهمیت خبر بدهید و بگویید چرا مخاطب باید آن را بداند."
    return f"خلاصه فارسی: {coverage}{context} {angle} منبع اصلی: {source_text}."
